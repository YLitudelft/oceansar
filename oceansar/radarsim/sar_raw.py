...
#            beam_pattern = sinc_1tx_nrx(sin_az, ant_L, f0, 1, field=True)
        ...
        # IF CONSIDERING THE XTI PROCESSING
        if hasattr(cfg.sar, 'b_cross'):
            sr_surface_m = np.zeros((num_ch, np.size(surface.x),np.size(surface.x)))
            for ch in np.arange(num_ch, dtype=np.int):                        
                gr_m = surface.x + gr0 + cfg.sar.b_cross[ch] * np.cos(np.radians(cfg.sar.b_roll[ch]))
                sr_m, inc_m, _ = geosar.gr_to_geo(gr_m, alt)
                sr_m -= np.min(sr_m)
                inc_m = inc_m.reshape(1, inc_m.size)
                sr_m = sr_m.reshape(1, sr_m.size)
                sin_inc_m = np.sin(inc_m)
                cos_inc_m = np.cos(inc_m)
                sr0_m = sr0 + cfg.sar.b_cross[ch] * np.sin(inc_angle - np.radians(cfg.sar.b_roll[ch]))
                sin_az_m = az / sr0_m
                sr_surface_m[ch] = (sr_m - cos_inc_m * surface.Dz + az / 2 * sin_az_m
                         + surface.Dx * sin_inc_m + surface.Dy * sin_az_m)
        ...
        for ch in np.arange(num_ch, dtype=np.int):
            ...
            if do_hh:
                ...
                if hasattr(cfg.sar, 'b_cross'):
                    cr_phase = np.exp(-1j * k0 * (sr_surface_m[ch] - sr_surface_m[0]))
                    scene_bp *= cr_phase
                else:
